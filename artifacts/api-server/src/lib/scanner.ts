import { db, serversTable, expiredServersTable } from "@workspace/db";
  ...
  export async function purgeStaleServers(): Promise<void> {
    // Use lastSeen (not firstSeen) so only servers that have gone MISSING from
    // Roblox's API for longer than MAX_SERVER_AGE_SECONDS get pruned.
    // Live servers always have their lastSeen refreshed every scan so they are
    // never affected. Also move stale servers to expired_servers so history is preserved.
    const nowMs = Date.now();
    const cutoff = nowMs - MAX_SERVER_AGE_SECONDS * 1000;

    const stale = await db
      .select()
      .from(serversTable)
      .where(lt(serversTable.lastSeen, cutoff));
    ...
    if (stale.length > 0) {
      logger.info({ purged: stale.length }, "Purged stale servers (moved to expired)");
    }
  }
